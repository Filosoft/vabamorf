...
import elg_estnltk_tokenizer

'''
# command line script
./create_venv.sh
venv/bin/python3 ./elg_sdk_tokenizer.py --json='{"params":{"placeholder": "app specific flags"},"type":"text","content":"Mees peeti kinni. Sarved&Sõrad"}'
...
class EstNLTK_tokenizer(FlaskService):
    def process_text(self, request) -> TextsResponse:
        ...
        request.annotations["sentences"], request.annotations["tokens"] = elg_estnltk_tokenizer.estnltk_lausesta_text4elg(request.content)   
...
if __name__ == '__main__':
    ...
    $ curl --silent --request POST --header "Content-Type: application/json" --data '{"type":"text","content":"Mees peeti kinni. Vanaisa tööpüksid."}' localhost:5000/process | jq
    ...
    $ venv/bin/python3 ./docker_elg_estnltk_tokenizer.py --indent=4 --json='{"content":"Mees peeti kinni. Vanaisa tööpüksid."}'
    $ venv/bin/python3 ./docker_elg_estnltk_tokenizer.py --indent=4 --json='{"features":{"optional":"optional"},"content":"Mees peeti kinni. Sarved&Sõrad","annotations":{"optional":[{"start":0,"end":4},{"start":5,"end":10}]}}'
    '''
    import argparse
    import json
    import sys
