...
* ```pref.pok``` - eesliited
* ```loend1```-```loend10``` (seejuures ```loend9``` ei ole) - mitmesugused sõnasarnaste 
...
* ```#``` abil on eristatud vorm, mis on (ainsuse nimetava asemel) selle sõna semantiline algvorm; käändsõnade puhul 
...
Kohustuslik element on ainult sõnaliik: muutumatutel sõnadel muuttüüp puudub. nt. ```vvsm:0093|kaasas!\D\!```
...
Näiteks: ```vvs:0093|vedru!\S\.16! **$SG.P$: vedru & vedru[T``` 
